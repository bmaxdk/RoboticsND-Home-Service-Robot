...
using namespace std;
// need move_base_msgs, actionlib, and roscpp dependencies
...
int main(int argc, char** argv)
{
    ...
    vector<vector<double>> waypoints = {{5.0, -6.0}, {12.0, -2.0}, {0.0, 0.0}};
    map<uint, string> station_map = {{0, "First Waypoint"}, {1, "Second Waypoint"}, {2, "Third Waypoint"}};
    ...
    for(auto &wp : waypoints)
    {
        ...
        if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
            ...
            ROS_INFO("The base failed to reach %s for some reason!!\n", station_map[count].c_str());
        count += 1;
    }
    return 0;
}